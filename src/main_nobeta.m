   ...
   disp(['iters: ' num2str(iters)]); 
    spmd(N)  
        for i = drange(1:N)
            ...
            theta = m_step_full(theta, optim_options, N, w_bound, w(i,:), squeeze(h(i,:,:,:)), n, i,...
    ...
    spmd
        for i = drange(1:N)
           llv(i,1) = log_likelihood(i, b(i), w(i,:), squeeze(h(i,:,:,:)),n,delta,squeeze(p_weights(i,:,:)));
        end
    end
    nll = sum(gather(llv));
        ll = [ll nll];
    ...
    save([data '_beta.mat'], 'iters','sigma', 'tau', 'delta', 'w_gathered', 'b_gathered','data','ll');
