...
while(norm(w - w_prev) > thresh_w)    
    ...
    spmd(N)  
        for i = drange(1:N)
            ...
            while(theta_intrinsic(2) - old_theta_intr(2) > .1 || norm(theta_intrinsic([1 3:end]) - old_theta_intr([1 3:end])) > 0.01)
                ...
                theta_intrinsic = m_step_smc_j(theta_intrinsic, optim_options, beta_subset, w(i,:), squeeze(h(i,:,:,:)), n, i, delta, tau, sigma, squeeze(p_weights(i,:,:)));                
    ...
    spmd(N)
        for i = drange(1:N)
            ...
            beta_subset = reshape(beta(i,:,:), N, S - 1);
            theta = m_step_full_j(theta, optim_options, N, beta_subset, w(i,:), squeeze(h(i,:,:,:)), n, i, delta, tau, sigma, squeeze(p_weights(i,:,:)));
